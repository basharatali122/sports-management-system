...
#         return response_handler(error=f"Error in get events: {str(e)}", status_code=500)
...
# #             'team': ObjectId(data.get('team')) if data.get('team') else None,
# #             'coach': ObjectId(data.get('coach')) if data.get('coach') else None,
# #             'participants': [ObjectId(p) for p in data.get('participants', [])],
# #             'createdBy': ObjectId(request.user['_id']),
# #             'organizer': request.user.get('name'),
# #             'approved': False
# #         }
...
#             'userId': ObjectId(request.user['_id'])
#         })
...
#         Event.add_participant(event_id, request.user['_id'])
...
#         Event.remove_participant(event_id, request.user['_id'])
        
#         # Delete registration
#         EventRegistration.find_one_and_delete({
#             'eventId': ObjectId(event_id),
#             'userId': ObjectId(request.user['_id'])
#         })
...
#     try:
#         coach_id = ObjectId(request.user['_id'])
...
#         result = []
#         for event in events:
#             event_regs = [r for r in registrations if r['eventId'] == event['_id']]
#             event['_id'] = str(event['_id'])
