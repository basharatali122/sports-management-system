...
//       try {
//         setLoading(true);
//         const response = await axios.get("http://localhost:3000/users/getAllparticipant");
...
function ManageUser() {
  ...
  {user.role === "participant" && !user.approvedByCoach ? (
    <div className="flex gap-2">
      ...
      <button
        onClick={() => handleRejection(user._id || user.id, user.role)}
        className="inline-flex items-center px-3 py-1.5 bg-rose-600 text-white text-sm font-medium rounded-lg hover:bg-rose-700 transition-colors"
      >
        Reject
      </button>
    </div>
  ) : (
    <span className="inline-flex items-center px-3 py-1.5 bg-slate-100 text-slate-500 text-sm font-medium rounded-lg">
      Approved
    </span>
  )}
</td>
